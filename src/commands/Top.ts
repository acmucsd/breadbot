...
export default class Top extends Command {
  ...
  public async run(interaction: CommandInteraction): Promise<void> {
    ...
    if (interaction.options.get('members') !== null) {
      leaderboardSize = Number(interaction.options.get('members'));
    }
    ...
    if (interaction.options.get('type') !== null) {
      leaderboardType = String(interaction.options.get('type'));
    }
    ...
    try {
      ...
      const leaderboardEmbeds: EmbedBuilder[] = [];
